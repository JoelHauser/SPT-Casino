...
///
/// Records what was taken for a race and not yet returned. Unlike Poker's, which
...
public sealed class FakeEscrow : IEscrowStore
{
    ...
    /// would look like to the next session, with no slip to go with it.
    /// </summary>
...
/// <summary>A race that runs the same way every time. Seeded, so a run is repeatable.</summary>
...
public sealed class QuietLog : IRaceLog
{
