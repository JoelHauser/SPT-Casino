...
namespace Roulette.Client
{
    ...
    internal static class ProfileSync
    {
        ...
        ///
        /// Must stay in step with `RouletteActions.Sync` on the server.
        /// </summary>
        private sealed class SyncOperation
        {
            public string Action = "RouletteSync";
        ...
        internal static void Request()
        {
            ...
            catch (Exception error)
            {
                ...
                RouletteClientPlugin.Log.LogError($"[Roulette] could not ask the game to resync: {error}");
            }
        }
        ...
        private static void OnSynced(IResult result)
        {
            if (result != null && result.Failed)
            {
                RouletteClientPlugin.Log.LogWarning(
                    $"[Roulette] the game refused the resync: {result.Error}. "
