...
{
    // Named per table, like escrow-horseracing.json: all four tables share one mod
    // folder now, and a bare stats.json would be Blackjack and this table
    // overwriting each other's record.
    private const string FileName = "stats-horseracing.json";
    ...
    // Writes are serialised: a race settling on one thread while another reads
        ...
        ModHelper modHelper)
    {
        ...
        {
            Writable = false;
            _logger.Error($"Horse racing: stats folder is not writable at {_path} -- {ex.Message}");
        }
    }
    ...
    public void Save(MongoId sessionId, PlayerStats stats)
    {
        ...
        lock (_writeLock)
        {
            ...
            {
                // Losing stats is annoying; losing the race because stats failed to
                // write would be worse. Log and carry on.
                _logger.Error($"Horse racing: could not write stats to {_path} -- {ex.Message}");
            }
        }
    }
    ...
    private ConcurrentDictionary<string, PlayerStats> Load()
    {
        ...
        catch (Exception ex)
        {
            ...
            // old file is overwritten on the next settled race.
            _logger.Error($"Horse racing: stats file at {_path} is unreadable, starting fresh -- {ex.Message}");
            return new ConcurrentDictionary<string, PlayerStats>();
        }
    }
}
