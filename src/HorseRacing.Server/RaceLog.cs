...
{
    /// <summary>
    /// Logs every request, every bet placed and every race. Noisy in
...
/// picked out of a busy server console with one filter on "[Races]".
/// </summary>
[Injectable(InjectionType.Singleton)]
public class RaceLog : IRaceLog
{
    private const string Prefix = "[Races]";

    private readonly ISptLogger<RaceLog> _logger;

    public RaceLog(ISptLogger<RaceLog> logger, ModHelper modHelper, FileUtil fileUtil, JsonUtil jsonUtil)
    {
        _logger = logger;
        ...
        var configPath = System.IO.Path.Combine(ModFolder, "horseracing.config.json");

        try
        {
            Config = fileUtil.FileExists(configPath)
                ? jsonUtil.Deserialize<RaceConfig>(fileUtil.ReadFile(configPath)) ?? new RaceConfig()
                : new RaceConfig();
        }
        catch (Exception ex)
        {
            ...
            _logger.Error($"{Prefix} horseracing.config.json is unreadable, using defaults -- {ex.Message}");
        }
    }
    ...
    void IRaceLog.Error(string message) => Error(message);
    ...
    // pockets is worth reading; a race's engine has nothing to say that the result
