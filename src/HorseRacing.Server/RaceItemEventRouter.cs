...
public static class RaceActions
{
    /// <summary>
    ...
    /// </summary>
    public const string Sync = "RacesSync";
}
...
/// when it wants them.
///
/// **The race deliberately stays on the static route.** A race takes several seconds
/// to run, and settling it through an item event would have the game apply the profile
/// change the instant the reply landed -- so the rouble counter behind the panel would
/// show the payout while the horses were still running. Roulette learned that with its
/// wheel and Slots with its reels; the fix is for the client to decide when to ask,
/// which is what this is.
/// </summary>
[Injectable(TypePriority = OnLoadOrder.Routers)]
public sealed class RaceItemEventRouter(RaceItemEventCallbacks callbacks)
    : ItemEventRouter([
        new ItemRouteAction<RaceSyncAction>(
            RaceActions.Sync,
...
/// interrupted race, and this has an output to hang that on.
/// </summary>
[Injectable]
public class RaceItemEventCallbacks(RaceService service, RaceLog log)
{
